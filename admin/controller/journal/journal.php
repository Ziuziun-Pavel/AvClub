    ...
    protected function getForm()
    {
        ...
        if (isset($case['company_id'])) {
            ...
            if (!empty($company)) {
                ...
                $company_info = $this->model_company_company->getCompany($company);
...
//template 1/0 полный/неполный
        // case title
        if ($this->request->post['type'] === 'case') {
            if ($this->request->post['case']['template'] === "1" && empty($this->request->post['company_id'])) {
                $this->error['case_company'] = $this->language->get('error_case_company');
            }

            if ($this->request->post['case']['template'] === "0" && empty($this->request->post['case']['title'])) {
