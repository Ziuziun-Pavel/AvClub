...
class ModelCatalogDownload extends Model {
	...
	public function getDownloadByName($download_name) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "download d LEFT JOIN " . DB_PREFIX . "download_description dd ON (d.download_id = dd.download_id) WHERE dd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND LCASE(dd.name) = '" . $this->db->escape(utf8_strtolower($download_name)) . "'");
