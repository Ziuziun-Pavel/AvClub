		...
		if($expert_info && ( ($status_logged && $this->request->get['route'] === 'register/account') || !empty($expert_info['expert']) ) ) {
			...
			if(!empty($this->request->get['tab'])) {
				switch(true) {
					...
					default: $show_tab = $this->request->get['tab'];
				}
			}else{
				$show_tab = '';
			}
			...
			if($tabs) {
				...
				foreach($tabs as $key=>$tab) {
					...
					switch($key) {
						case 'all':	$title = 'все';	break;
						case 'news':	$title = 'новости';	break;
						case 'article':	$title = 'статьи';	break;
						case 'opinion':	$title = 'мнения';	break;
						case 'video':	$title = 'видео';	break;
						case 'case':	$title = 'кейсы';	break;
						case 'online':
					...
					if(in_array($key, array('master', 'online')) && is_array($tab)) {
						...
						foreach($tab as $type) {
							switch($type) {
								case 'master_all':	$child_title = 'Ближайшие';	break;
								case 'master_master':	$child_title = 'Мастер-классы';	break;
								case 'master_meetup':	$child_title = 'Митапы';	break;
								case 'master_old':	
