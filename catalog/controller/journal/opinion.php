<?php
class ControllerJournalOpinion extends Controller {
	public function index() {
		...
		$meta_info = $this->config->get('av_meta_opinion');
		...
		$limit = $this->config->get('av_limit_opinion');
		...
		$pagination->url = $this->url->link('journal/opinion', 'page={page}');
		...
		if ($page == 1) {
			$this->document->addLink($this->url->link('journal/opinion', '', true), 'canonical');
		} elseif ($page == 2) {
			$this->document->addLink($this->url->link('journal/opinion', '', true), 'prev');
		} else {
			$this->document->addLink($this->url->link('journal/opinion', 'page='. ($page - 1), true), 'prev');
		}

		if ($limit && ceil($journal_total / $limit) > $page) {
			$this->document->addLink($this->url->link('journal/opinion', 'page='. ($page + 1), true), 'next');
		}
		...
		$this->response->setOutput($this->load->view('journal/opinion', $data));
