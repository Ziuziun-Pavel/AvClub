<?php
class ControllerJournalNews extends Controller {
	public function index() {
		...
		$meta_info = $this->config->get('av_meta_news');
		...
		$limit = $this->config->get('av_limit_news');
...
//        die();
		...
		$pagination->url = $this->url->link('journal/news', 'page={page}');
		...
		if ($page == 1) {
			$this->document->addLink($this->url->link('journal/news', '', true), 'canonical');
		} elseif ($page == 2) {
			$this->document->addLink($this->url->link('journal/news', '', true), 'prev');
		} else {
			$this->document->addLink($this->url->link('journal/news', 'page='. ($page - 1), true), 'prev');
		}

		if ($limit && ceil($journal_total / $limit) > $page) {
			$this->document->addLink($this->url->link('journal/news', 'page='. ($page + 1), true), 'next');
		}
		...
		$this->response->setOutput($this->load->view('journal/news', $data));
