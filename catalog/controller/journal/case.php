<?php
class ControllerJournalCase extends Controller {
	public function index() {
		...
		$meta_info = $this->config->get('av_meta_case');
		...
		$limit = $this->config->get('av_limit_case');
		...
		$pagination->url = $this->url->link('journal/case', 'page={page}');
		...
		if ($page == 1) {
			$this->document->addLink($this->url->link('journal/case', '', true), 'canonical');
		} elseif ($page == 2) {
			$this->document->addLink($this->url->link('journal/case', '', true), 'prev');
		} else {
			$this->document->addLink($this->url->link('journal/case', 'page='. ($page - 1), true), 'prev');
		}

		if ($limit && ceil($journal_total / $limit) > $page) {
			$this->document->addLink($this->url->link('journal/case', 'page='. ($page + 1), true), 'next');
		}
		...
		$this->response->setOutput($this->load->view('journal/case', $data));
