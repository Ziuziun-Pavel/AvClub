<?php
class ControllerJournalArticle extends Controller {
	public function index() {
		...
		$meta_info = $this->config->get('av_meta_article');
		...
		$limit = $this->config->get('av_limit_article');
		...
		$pagination->url = $this->url->link('journal/article', 'page={page}');
		...
		if ($page == 1) {
			$this->document->addLink($this->url->link('journal/article', '', true), 'canonical');
		} elseif ($page == 2) {
			$this->document->addLink($this->url->link('journal/article', '', true), 'prev');
		} else {
			$this->document->addLink($this->url->link('journal/article', 'page='. ($page - 1), true), 'prev');
		}

		if ($limit && ceil($journal_total / $limit) > $page) {
			$this->document->addLink($this->url->link('journal/article', 'page='. ($page + 1), true), 'next');
		}
		...
		$this->response->setOutput($this->load->view('journal/article', $data));
