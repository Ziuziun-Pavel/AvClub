			...
			<div class="expert__data col-md-8">
				...
				<div class="expertnav" data-id="<?php echo $expert_id; ?>">
					...
					<?php if(!empty($logged)) { ?>
						...
						<div class="expertnav__tabs">
							...
							<?php if($tabs) { ?>
								<a href="#" class="expertnav__tab link <?php echo !$active_tab ? 'active' : ''; ?>" data-type="article">Мои выступления в АВ Клубе</a>
								<?php $active_tab = true; ?>
							<?php } ?>

							<?php if(!$tabs) { ?>
								<a href="#" class="expertnav__tab link <?php echo !$active_tab ? 'active' : ''; ?>" data-type="bio">Биография</a>
								<?php $active_tab = true; ?>
							<?php } ?>
					...
					<?php $active_tab = false; ?>
					<?php if($tabs) { ?>
						<div id="navlist-article" class="expertnav__list <?php echo !empty($active_tab) ? '' : 'active'; ?>">
							<ul class="list-hor">
								<?php foreach($tabs as $key=>$tab) { ?>
									<li><a href="#" class="expertnav__change <?php echo $tab['active']; ?>" data-type="<?php echo $tab['key']; ?>"><?php echo $tab['title']; ?></a></li>
								<?php } ?>
							</ul>
										...
										<?php if(!empty($tab['children'])) { ?>
											<ul class="list-hor">
												<?php foreach($tab['children'] as $key_child=>$child) { ?>
													<li><a href="#" class="expertnav__change <?php echo $child['active']; ?>" data-type="<?php echo $child['key']; ?>"><?php echo $child['title']; ?></a></li>
												<?php } ?>
											</ul>
										<?php } ?>
									</div>
					...
					<?php if(!empty($event_list)) { ?>
						<div id="navlist-register" class="expertnav__list <?php echo !empty($active_tab) ? '' : 'active'; ?>">
							<ul class="list-hor">
								<li><a href="#" class="expertnav__change disable" data-type="">&nbsp;</a></li>
							</ul>
						</div>
				...
				<div class="expertrow row">
					...
					<div class="expertrow__content col-xl-8">
						...
						<?php if(!empty($tabs)) { ?>
							<div id="content-article" class="expert__content <?php echo !empty($active_tab) ? '' : 'active'; ?>">
								<?php $active = true; ?>

								<?php foreach($tabs as $tab) { ?>
									<?php $active = false; ?>
									<div class="expert__pane expert__pane-<?php echo $tab['key']; ?> <?php echo $tab['active']; ?>" data-key="<?php echo $tab['key']; ?>">
										...
										<?php if(!empty($tab['children'])) { ?>
											<?php foreach($tab['children'] as $child) { ?>
												<div class="expert__pane expert__pane-<?php echo $child['key']; ?> <?php echo $child['active']; ?>" data-key="<?php echo $child['key']; ?>">
						...
						<?php if(!$tabs && !empty($bio)) { ?>
							<div id="content-bio" class="expert__content <?php echo !empty($active_tab) ? '' : 'active'; ?>">
								<?php require(DIR_TEMPLATE . 'avclub/template/expert/expert_info-bio.tpl'); ?>
							</div>
							<?php $active_tab = true; ?>
						<?php } ?>
						...
						<?php if(!empty($event_list)) { ?>
							<div id="content-register" class="expert__content <?php echo !empty($active_tab) ? '' : 'active'; ?>">
